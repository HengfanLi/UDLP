...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    ...
    image_path="../data/2018LA_Seg_Training Set/ZQPMJ4XEC5A4BISD45P1/mri_norm2.h5"
...
def test_single_case_first_output(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    #print()
    uncertainty = -1.0*torch.sum(score_map*torch.log(score_map + 1e-6), dim=0, keepdim=True)
    print('uncer: ',uncertainty.shape)
    #plt.savefig('./1/pic.png')
    plt.imshow(uncertainty[0,:,:,40], cmap='jet', interpolation='nearest')
    #plt.colorbar()
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                # uncertainty = -1.0*torch.sum(score_map*torch.log(score_map + 1e-6), dim=0, keepdim=True)
                # # 获取切片的不确定性地图
                # print(' uncertianty: ',uncertainty.shape)
                # uncertainty_slice = uncertainty[0,  :, :, 50].cpu().numpy()
                
                # # 绘制不确定性地图
                # plt.imshow(uncertainty_slice, cmap='jet', interpolation='nearest')
                # plt.colorbar()
                # plt.title('Uncertainty Map')
                # plt.show()
                cnt[xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] \
