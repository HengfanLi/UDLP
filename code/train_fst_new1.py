...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #pseudo_mask = (ema_output_soft > max_probs_thresholds).float()
            ...
            #print('uncertainty.requires_grad ',uncertainty.requires_grad)
            preds1 = preds.clone()
            ...
            #consistency_dist = (outputs_u_soft-ema_output_soft_mask1)**2
            threshold = (0.75+0.25*ramps.sigmoid_rampup(iter_num, max_iterations))*np.log(2)
            mask = (uncertainty<threshold).float()
            #print(' mask: ',mask)
            #consistency_dist= (F.cross_entropy(outputs[labeled_bs:], pseudo_outputs1, reduction='none') * mask1).mean()
...
###############################################################################
            ...
            #with torch.no_grad():
            # prob = torch.softmax(ema_output.detach(), dim=1)
            # entropy = -torch.sum(prob * torch.log(prob + 1e-10), dim=1)
            # ema_model.eval()
            # outputs2 = ema_model(unlabeled_volume_batch)
            # outputs_soft_label_u = torch.softmax(outputs2, dim=1)
            # _, label_u = torch.max(outputs_soft_label_u, dim=1)
        ...
        # ema_model.train()
            ...
            # thresh1 = np.percentile(entropy[target!=2].detach().cpu().numpy().flatten(), drop_percent)
            # thresh_mask = entropy.ge(thresh1).bool()*(target != 2).bool()
            ...
            # if (iter_num%400==0 and iter_num<=4000):
            #     max_probs_thresholds=max_probs_thresholds-0.01    
            #print('max_probs_thresholds: ',max_probs_thresholds)
